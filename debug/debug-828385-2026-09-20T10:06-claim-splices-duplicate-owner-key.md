...
- 2026-09-21T14:55-06:00 — phase 4: RETROACTIVE and POST-MERGE (PR #162 merged as `6b8684bd` without one; hand-run through the helpers, which never spawn review agents). Shape: three-agent parallel adversarial review — Blind Hunter (diff only) + Edge Case Hunter (adversarial inputs + a 320-spec corpus scan) + Acceptance Auditor (every AC). Findings: Blind 4, Edge 7, Auditor 11 — about 16 unique after overlap; 0 HIGH, 5 MED. The MEDs: `manage/loop.ts` destroyed the line after a bare `status:` and reported success; `validate-emit` read the next line for a bare key; a `--- ` closing fence regressed to "no frontmatter"; deleting a duplicate that heads a block orphaned its lines (silent corruption for a block scalar); `specDuplicateKeys` was never printed. None of the broken shapes exists in any real spec. Also found this spec's HALT consequence never happened. All fixed forward in `debug-108c57` / PR #171, which got its own pre-merge three-agent review.
