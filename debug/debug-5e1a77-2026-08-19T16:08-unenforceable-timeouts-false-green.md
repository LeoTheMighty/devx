...
- 2026-08-20T17:36:09.848Z — loop iteration 3: Traced most of the blocking suite's wall-clock to macOS PATH-resolution cost (~150ms per bare-name spawn), fixed it in the exec seam and the test fixtures, and cut the blocking pass 1,024s → 155s with over-cap tests dropping 16 → 3.
  - Change: src/lib/exec.ts grows resolveCommandPath: both realExec and realExecAsync now resolve a bare command name to an absolute path once per (command, effective-PATH) pair, keyed on opts.env.PATH so an override is never served a stale answer; deliberately conservative — Windows, a name with a separator, an empty PATH entry (cwd) or a relative entry all hand the question back to execvp unchanged
  - Change: test/helpers/git-bin.ts (new) exports a once-resolved GIT reusing the production resolver; the shared loop fixture and the 14 test files that drove git directly now spawn it by absolute path, and makeFixture's per-spec `mkdir` subprocess became mkdirSync
  - Change: 6 regression tests for resolveCommandPath in test/exec-async-seam.test.ts covering first-wins ordering, directory/non-executable shadowing, the bail-out cases, cache keying, both seams honouring an opts.env PATH override, and 127 for a missing binary — negative-controlled three ways (name-only cache key, dropped isFile check, dropped empty-entry bailout each turn it red; control green)
  - Change: scripts/timeout-headroom.mjs LAST SWEEP block rewritten with the 2026-08-20 two-pass distribution (AC 3), the prior sweep demoted to PRIOR SWEEP, the three remaining over-cap tests named, the two deliberate sub-1x rows flagged as read-past, and an explicit statement that fault (2) is still open
  - Change: vitest.shared.ts records the PATH measurement and a 'suspect the machine first' warning with the 674s-vs-172s evidence
  - Learning: The dominant cost in the blocking pass was never the blocking — it was PATH resolution. spawnSync("git") = 154ms vs spawnSync("/usr/bin/git") = 9.7ms on a 54-entry PATH; 25 nonexistent dirs + /usr/bin reproduces it at 151ms, so it is ~5.6ms per FAILED execve attempt. Identical for async spawn (158ms → 12ms), unaffected by the Claude Code sandbox, and unaffected by parent RSS (flat 155ms from 49MB to 2GB — that hypothesis is dead).
  - Learning: Iteration 2's 'severe and unexplained within-file amplification' was macOS sleep. Two consecutive runs of the UNCHANGED loop-driver.test.ts measured 674s and 172s; the 674s run had one test at 501s with the whole file at 8% CPU. Any test duration that looks insane and shows near-zero CPU should be re-measured before it is diagnosed.
  - Learning: devx-claim.test.ts now runs 60 tests in 879ms — the spec's 114,868ms row for :754 is fully retired by iteration 2's hook fix, and the PATH fix adds nothing there because it barely spawns git. Isolation numbers for that file are no longer the story.
  - Learning: Do not benchmark a suite change on loop-driver's full file: run-to-run variance is 4x. A named subset (-t "runLoop scenarios") run twice per arm gave stable, reproducible A/B numbers.
  - Learning: /bin/true does not exist on macOS (only /usr/bin/true), so it is a trap as a cross-platform test fixture — it would have passed on Linux CI and failed only locally. Two tmpdir shims of the same name assert 'first wins' without a platform assumption.
  - Learning: The three remaining over-cap tests are two real-PREDICATE git hooks (loop-driver:863 commit-failure, :554 push failure at acs_met) plus init-e2e:890. The symlink trick from iteration 2 does not apply directly because both hooks must let the claim's main push through while rejecting the feature push — arming the free symlink mid-run from the scripted worker is the likely route, but it needs a check that the abandon path's own main push is not collaterally rejected.
- 2026-08-20T18:11:58.511Z — loop iteration 4: Adopted the async exec seam in the loop driver — negative-controlled proof that a runLoop test's declared timeout now actually fires — and closed the last accidental over-cap tests, taking the suite from 3 to 0 with the AC-3 sweep rewritten.
  - Change: src/lib/loop/git-tx.ts is async end-to-end and typed against a new ExecLike (Exec | ExecAsync) union in exec.ts; driver.ts defaults to realExecAsync and preflight.ts's probeMainHealth follows. ~40 driver call sites and 12 internal closures went async. Zero synchronous test fakes changed, because await on a non-promise is a no-op — that union is what made the migration cheap.
  - Change: Two seams deliberately stay synchronous, with the reason recorded at the definition: work held under withBacklogLock (the lock acquires/releases around a SYNCHRONOUS callback, so awaiting inside it would release the lock with a child still running — the lost-update race the lock exists to stop) and claimSpec + the merge tail (withGhRetry/checkHold busy-wait; once per item, not per iteration). driver.ts carries an explicit execSync seam for exactly those.
  - Change: New test/loop-driver-timeout-enforcement.test.ts: two independent proofs that the driver's caps now fire — a tick counter (0 before, positive now) and an it.fails case that passes only because a 100ms cap interrupts a real runLoop. Negative-controlled: reverting the one default in driver.ts turns the file red on both.
  - Change: The six hook-bearing loop-driver tests carry one shared explicit cap (HOOK_TEST_TIMEOUT_MS = 120s) with the measurement that justifies it; init-e2e:890 carries an explicit 30s cap labelled in place as still-unenforceable rather than presented as fixed. No assertion was weakened (AC 4).
  - Change: scripts/timeout-headroom.mjs LAST SWEEP block rewritten from a fresh two-pass sweep (AC 3): accidental over-cap tests 16 → 3 → 0, under-2x 21 → 9 → 5, under-5x 65 → 52 → 51, with the three surviving sub-1x rows named as deliberate. vitest.shared.ts's fault-(2) note upgraded from 'adoption has not happened' to the adoption record plus the remaining surface.
  - Learning: Making the caps enforceable immediately turned three silent passes into three RED timeouts in the full blocking pass — the fix's first act was to surface the very failures the spec was filed about. Any future iteration doing this should expect the suite to go red at the moment the mechanism lands, and budget for triaging what falls out.
  - Learning: The backlog lock is the real boundary on this migration, not the call count. withBacklogLock wraps a synchronous callback, so 'just await everything' would silently release the lock while a git child ran — the exact lost-update race it exists to prevent. Splitting the driver into an async seam plus a small execSync seam for lock-held bookkeeping is what kept the change safe; a blanket conversion would have been a correctness regression that no test would have caught.
