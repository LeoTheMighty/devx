...
- 2026-08-12 — phase 2/3 (root cause, AC 1). What the three failing tests await: `await runManagerOnce(...)` → `spawnWorker` spawns the stub `claude` shell script (a real child, `sleep 0.1`), awaits its `exit`, then writes the roster/state JSON. That whole round trip is the awaited step. **Idle vs loaded, same commit, same 12-core macOS box:**
  - whole file in ISOLATION: 6 tests, **1.32s** total; the slowest async test **355ms**.
  - same file inside pass 2 (`maxForks: 2`, alongside `loop-driver.test.ts` at 561s of solid spawnSync): **3 failed**, all `Test timed out in 5000ms`; in-file sibling at **52,131ms**.
  A ~14x amplification on a 355ms test is contention, not intrinsic cost. 5s is not "too tight" for the work — the work is 355ms; the process simply isn't scheduled.
  ROOT CAUSE (confirmed): the sync-blocking partition (debug-7c1e93) is per-FILE, but blocking is per-TEST. This file had ONE `spawnSync` test (`:316`) and five `await runManagerOnce` tests, so the single blocker dragged all five into pass 2's starved lane.
  **REFUTES the note recorded in `vitest.blocking.config.ts`** ("it starves itself, so no partition can rescue it, that one needs the exec-seam fix"). The file's own sync share is ~1s of a 1.32s isolated run — it cannot hold a 355ms sibling past 5s. The starvation was cross-file, and a partition did rescue it. That comment is corrected in-place, because it was actively pointing the next reader at an expensive and unnecessary exec-seam refactor.
- 2026-08-12 — phase 3 (fix, AC 2). Fixed at the mechanism, with **no timeout touched anywhere** — the b7f2c1 rule holds. The one `spawnSync` test moved to `test/manage-spawn-cli-e2e.test.ts` (stays pass 2); the five async tests stay in `test/manage-spawn-integration.test.ts`, which now references no synchronous child-process API and therefore moves to pass 1 by the partition's own mechanical rule. `SYNC_BLOCKING_TESTS` swapped accordingly; `test/vitest-split.test.ts` derives membership from the tree and passes, so it now pins the new arrangement in both directions.
- 2026-08-12 — phase 3 (AC 3, regression under contention). Not an idle-box green: re-ran with 12 `yes` processes saturating all 12 cores (1-min load average reached **13.67**). `manage-spawn.test.ts` + `manage-spawn-integration.test.ts` + `manage-spawn-cli-e2e.test.ts` + `loop-worker.test.ts` = **56/56 passed** in 19.61s (vs 1.44s idle — a 13x slowdown that the tests absorbed without a single timeout). Load generators confirmed reaped afterwards.
- 2026-08-12 — phase 3 (AC 4, sibling audit). Enumerated all 26 files in `SYNC_BLOCKING_TESTS` by sync-call count vs async-test count. **No second instance of this shape exists**: `manage-spawn-integration` was uniquely 5 async tests held hostage by 1 sync test. The nearest neighbours are sync-dominant (`loop-iteration` 2 async / 41 sync, `spec-lock` 1/33, `worktree-refusal` 2/2), where extraction would buy nothing. The AC's named suspect, `loop-worker.test.ts`, contains **zero** synchronous child-process calls — it was never in pass 2, and its 5s-timeout failure was collateral starvation from pass 1's pre-partition blockers, already fixed by PR #124. Same class as the AC guessed; different fix, and already shipped.
