...
- [/] `debug/debug-b365ac-2026-07-15T12:14-yaml-devdep-breaks-tarball-install.md` — `yaml` is a devDependency but dist/lib/config-io.js imports it at runtime, so a clean packed-tarball install crashes with ERR_MODULE_NOT_FOUND on every invocation. Status: in-progress. From: debug-e3f1c2 verification (throwaway-prefix tarball install).
