...
- [ ] `dev/dev-d982ea-2026-09-20T10:37-duplicate-spec-warning-at-filing.md` — Warn at filing time when a new spec's title or cited test paths substantially overlap an OPEN row. palateful's `imptb1`/`imptab1` were the same bug, same three failing tests, hashes one character apart, filed three minutes apart by two sessions that couldn't see each other; `imptab1` was fixed that evening, `imptb1` sat open and unowned for seven weeks until a tab was dispatched against it and stood down. The signal was already in hand — both rows in one file, three minutes apart, nothing compared them. Warns, never blocks; runs off data `devx next` already loads; AC 4 requires a measured false-positive rate before the threshold is fixed. Justified on **longest tail, not biggest cost** — the realized cost was one wasted dispatch, far less than the 51-day freeze. Status: ready. From: palateful-0e via coordinator session 2026-09-20.
...
- [ ] `dev/dev-9946f9-2026-07-30T14:07-human-smoke-of-the-devx-learn-watch-terminal-app-s.md` — Human smoke of the devx learn-watch Terminal.app spawn arm. Status: ready. Blocked-by: rtl104. **Re-homed 2026-09-04**: retro-listener closed; this row stands alone (human smoke, no agent work).
- [ ] `dev/dev-dlrret-2026-09-02T09:18-retro-docs-layout-resolution.md` — Retro + LEARN.md updates (interim retro discipline). Status: ready. Blocked-by: dlr101, dlr102, dlr103, dlr104, dlr105, dlr106, dlr107. **Re-homed 2026-09-04**: docs-layout-resolution closed; retro still worth running — reads `_devx/archive/docs-layout-resolution/`, which still resolves.
...
- [x] `dev/dev-5c215e-2026-09-21T12:10-s1-full-run-prose-budget.md` — S-1: give the full-run prose surface its own deliberate budget (separate knob from the planning gate; replaces the 2× drift tripwire; D-14; answers INTERVIEW Q#9). Status: done. Blocked-by: —. PR: https://github.com/LeoTheMighty/devx/pull/168 (merged 8ad6324)
...
- [x] `dev/dev-dlr103-2026-09-02T09:14-workstream-resolution-flat-guard.md` — Docs layout resolution phase 3: workstream resolution reaches the repo root under project-level (3 frontmatter states), `planFilenameWorkstreamRel()` re-signatured, the one flat-era guard that genuinely misfires discriminated, `layout-tree-mismatch` doctor finding. Ships no new user-reachable state (R-2). Blocked-by: dlr101. Parallel-safe with dlr102. Status: done. PR: https://github.com/LeoTheMighty/devx/pull/153 (merged aa35a60)
- [x] `dev/dev-dlr104-2026-09-02T09:14-consumer-sweep-scaffolding.md` — Docs layout resolution phase 4: the consumer sweep (ten `*Abs()` helpers over the resolved base, 21 call sites, every hand-join closed, `devx next` probes) plus layout-aware scaffolding with the slug optional. E-3's scan is authored FIRST and negative-controlled against the live sites (R-6). No compile break. Blocked-by: dlr102, dlr103. Status: done. PR: https://github.com/LeoTheMighty/devx/pull/154 (merged 366a8f1)
- [x] `dev/dev-dlr105-2026-09-02T09:14-identity-rekey-privatization.md` — Docs layout resolution phase 5: `CASCADE_TABLE` re-keyed on `ArtifactKind`, `*_REL` + `artifactAbs` made private, orphaned resolvers deleted. THE RISKIEST PHASE — the only compile-breaking one, and where R-1 and R-4 live. Blocked-by: dlr104. Status: done. PR: https://github.com/LeoTheMighty/devx/pull/155 (merged 6747362)
- [x] `dev/dev-dlr106-2026-09-02T09:14-layout-migrate.md` — Docs layout resolution phase 6: `devx layout migrate --to <layout> [--dry-run]` — pure `MovePlan` planner, `git mv` executor, 3 refusals computed before any move, no `--force`. NOT REVERT-SAFE for a repo that ran it (R-5); G-3's real evidence is MANUAL.md MV-a494be.1 on ClassyLights. Blocked-by: dlr105. Parallel-safe with dlr107. Status: done. PR: https://github.com/LeoTheMighty/devx/pull/156 (merged 35a6ae0)
- [x] `dev/dev-dlr107-2026-09-02T09:14-doc-truth.md` — Docs layout resolution phase 7: doc truth — CONFIG.md's artifact table restructured to one row per `ArtifactKind` (13 rows; the 13-row set-equality is the RED-bearing half), the schema description rewritten, and a follow-up filed for the nine skill-body path references. Blocked-by: dlr105. Parallel-safe with dlr106. Status: done. PR: https://github.com/LeoTheMighty/devx/pull/157 (merged 9832854)
- [x] `dev/dev-a57f22-2026-09-02T15:10-skill-body-layout-paths.md` — Follow-up from dlr107: the nine skill-body path references that hardcode the folder shape (`.claude/commands/devx-plan.md` x6, `.claude/commands/devx.md` x3). Under `project-level` an agent following those skills WRITES into the folder shape while the CLI READS the flat one — the reader/writer split this workstream closed in code, one layer out in prose. Prefer naming the resolving CLI over restating a path. S-1 headroom re-measured at 6,102 bytes (not the single-digit figure dlr107's spec assumed). Status: done. From: dlr107. PR: https://github.com/LeoTheMighty/devx/pull/158 (merged 6b36317)
