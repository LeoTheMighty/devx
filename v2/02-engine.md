...
as is an exact move of an outline to the same stage's outline path (`devx
layout migrate`, `devx archive`); all three L2 sites share one
`scanOutlineDiff` and one `outlineDiffArgs`. Under `engine.docs_layout:
