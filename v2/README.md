...
*(The review tour shipped at V2.3 and was retired 2026-08-04 at `tur101` — see
`07-decisions.md` D-4. Everything else stands.)*
...
2. `LeoTheMighty/code-review-tour` — the guided-tour review UI we adapted
   into a self-contained static HTML artifact per PR. *(Retired at tur101.)*
...
| ~~`03-review-tour.md`~~ | **Deleted 2026-08-04 (tur101)** when the review tour was retired. What survived it — `/devx address <pr>` and the `devx: hold` merge gate — is specified in `05-dispatcher.md` and `07-decisions.md` D-5. |
...
mechanical gates, a **universal dispatcher** so `/devx` on any repo knows
whether to plan, design, execute, debug, or address review comments next, and an **overnight loop** with gnhf's transactional-git iteration
