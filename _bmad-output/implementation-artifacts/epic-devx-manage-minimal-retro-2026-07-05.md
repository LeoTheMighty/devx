...
### 3.2 roc101 (verify-claim) did not ship before mgr104 — the collision class dvxret named stayed live through the epic (closed mid-retro by PR #60)

dvxret E13 filed `dev-roc101` (resume-detection / verify-claim) as "load-bearing for mgr104's worker-spawn discipline." mgr104 shipped without it: spawned workers ran `claude /devx <hash>` with no structural ownership check, so the fresh-session-stomps-live-claim class was open for the entire epic (hard cap = 1 bounded the exposure — manager + at most one worker — but a human-invoked `/devx` alongside a manager-spawned worker reproduces dvxret's incident shape unattended). **Update mid-retro:** while this retro PR was in its CI cycle, a concurrent `/devx` session shipped roc101 via PR #60 (`devx devx-helper verify-claim` + Phase 1 resume-detection skill-body wire-up, merged 2026-07-05) — the structural gap is now closed, and the v2 universal dispatcher (`v2/05-dispatcher.md`) inherits the primitive rather than redesigning it. The sequencing lesson stands: the primitive named "load-bearing for mgr104" landed two months after mgr104; ordering-critical follow-ups should block the stories that need them, not trail them. Fittingly, this retro's own merge cycle collided twice with that concurrent session (see §3.8) — the exact class roc101/verify-claim exists to manage.
...
### 3.8 Two concurrent-session collisions during this retro's own merge cycle

This retro's PR (#61) went red/conflicted twice against a concurrent `/devx` session working the v2 backlog on the same repo: (1) first CI run failed on the merge ref because main had gained v2s101 (PR #59) whose done-flip lacked the canonical `phase 4:` status-log line — both sessions then fixed the same line independently (this branch's fix-forward commit vs roc101 PR #60's retroactive append), producing a textual merge conflict resolved in main's favor; (2) the first `gh pr merge` attempt failed with real merge conflicts because roc101 (PR #60) merged between this PR's CI-green and its merge command. Neither collision lost work, but both are exactly the multi-actor coordination class that roc101/verify-claim and the v2 dispatcher exist to manage — a live demonstration, during the final BMAD-era retro, of why Phase 1's hard-cap-1 assumption no longer holds in practice. Recorded as evidence for the v2 dispatcher design (`v2/05-dispatcher.md`).

### 3.9 Retro-row backfill — 10th and final confirmation
